...
def process_cabinet(path, scale, point_sample, selected_names) :
	...
	with open(semantic_file, "r") as f:
		...
		for link_repr in semantics :
			...
			if link_type == "slider" and link_name == "drawer":
				# found a rotation door
				new_cabinet_name = os.path.basename(path) + "_"  + link_id
				if new_cabinet_name not in selected_names :
					continue
				dst_path = os.path.join("dataset/one_drawer_cabinet", new_cabinet_name)
				total_handle += build_new_cabinet(path, dst_path, link_id, scale)
...
if __name__ == "__main__" :
	...
	with open("selected_drawer.yaml", "r") as f :
		selected_names = yaml.load(f, Loader=Loader)
	print("total", len(cabinet_name_list), "drawers")
	...
	with tqdm(total=total_len) as pbar:
		...
		for name in cabinet_name_list :
			...
			new_name_list, new_path_list, new_handle = process_cabinet(os.path.join(root, name), scale, point_sample, selected_names)
	...
	print("total", len(name_list), "new drawers", total_handle, "handles")
	...
	with open("./drawer_conf.yaml", "w") as f:
